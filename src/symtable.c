...
void add_builtin_functions()
{
    ...
    symtable_item item = {
        .id = "readString",
        .type = FUNCTION,
        .data.func_data = init_func_data()};
    item.data.func_data->return_type = TYPE_STRING;
    symtable_add(item, symtable_stack_top(sym_st));

    // func readInt() -> Int?
    item.id = "readInt";
    item.data.func_data->return_type = TYPE_INT;
    symtable_add(item, symtable_stack_top(sym_st));

    // func readDouble() -> Double?
    item.id = "readDouble";
    item.data.func_data->return_type = TYPE_DOUBLE;
    symtable_add(item, symtable_stack_top(sym_st));

    // func write ( term1 , term2 , ..., term𝑛 )
    item.id = "write";
    item.data.func_data->return_type = TYPE_NIL;
    item.data.func_data->params_count = -1;
    symtable_add(item, symtable_stack_top(sym_st));
    ...
    item.data.func_data->return_type = TYPE_DOUBLE;
    item.data.func_data->params_count = 1;
    item.data.func_data->params = malloc(sizeof(ParamData));
    item.data.func_data->params[0].name = "_";
    item.data.func_data->params[0].id = "term";
    item.data.func_data->params[0].type = TYPE_INT;
    symtable_add(item, symtable_stack_top(sym_st));
    ...
    item.data.func_data->params[0].name = "_";
    item.data.func_data->params[0].id = "term";
    item.data.func_data->params[0].type = TYPE_DOUBLE;
    symtable_add(item, symtable_stack_top(sym_st));
    ...
    item.data.func_data->params = malloc(sizeof(ParamData));
    item.data.func_data->params[0].name = "_";
    item.data.func_data->params[0].id = "s";
    item.data.func_data->params[0].type = TYPE_STRING;
    symtable_add(item, symtable_stack_top(sym_st));

    // func substring(of s : String, startingAt i : Int, endingBefore j : Int) -> String?
    item.id = "substring";
    item.data.func_data->return_type = TYPE_STRING;
    item.data.func_data->params_count = 3;
    item.data.func_data->params = malloc(sizeof(ParamData) * 3);
    item.data.func_data->params[0].name = "of";
    item.data.func_data->params[0].id = "s";
    item.data.func_data->params[0].type = TYPE_STRING;
    item.data.func_data->params[1].name = "startingAt";
    item.data.func_data->params[1].id = "i";
    item.data.func_data->params[1].type = TYPE_INT;
    item.data.func_data->params[2].name = "endingBefore";
    item.data.func_data->params[2].id = "j";
    item.data.func_data->params[2].type = TYPE_INT;
    symtable_add(item, symtable_stack_top(sym_st));
    ...
    item.data.func_data->params = malloc(sizeof(ParamData));
    item.data.func_data->params[0].name = "_";
    item.data.func_data->params[0].id = "c";
    item.data.func_data->params[0].type = TYPE_STRING;
    ...
    item.data.func_data->params = malloc(sizeof(ParamData));
    item.data.func_data->params[0].name = "_";
    item.data.func_data->params[0].id = "i";
    item.data.func_data->params[0].type = TYPE_INT;
    symtable_add(item, symtable_stack_top(sym_st));
}