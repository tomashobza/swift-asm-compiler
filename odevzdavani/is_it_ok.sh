...
#  2023-09-29  Upraveny identifikatory rozsireni pro IFJ23
...
if [[ -f rozsireni ]]; then
  ...
  RETCODE=$?
  if [[ $RETCODE = "0" ]]; then
    UNKNOWN=`cat rozsireni | grep -v -E -e "^(OVERLOAD|INTERPOLATION|BOOLTHEN|CYCLES|FUNEXP)$" | wc -l`
    if [[ $UNKNOWN = "0" ]]; then
      echo_color green "OK" 
    else
