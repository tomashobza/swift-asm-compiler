...
test_file() {
    local testinput="$(dirname "$0")/blaza_tests/$1"
    ...
    if [ -z "$2" ]; then
        "$(dirname "$0")"/../"$BIN" >/dev/null 2>/dev/null <"$testinput"
        RETURN_CODE=$?
    else
        "$(dirname "$0")"/../"$BIN" <"$testinput"
        RETURN_CODE=$?
    fi
...
for testfile in "$(dirname "$0")"/blaza_tests/*.swift; do
